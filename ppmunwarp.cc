...
// cover the picture area of interest. The he puts this calibration grid
...
// distance between three calibration points is added, but in that area
...
struct Parameter {
  ...
  // The percentual width of the hue area for calibration point detection:
  int pointRange;
...
static void convertRGBtoHSV(int r,int g,int b,double& h,double& s,double& v) {
  int i;
  // Gray values have arbitrary hue:
  if (r == g && r == b) { h = s = 0.0;  v = double(r) / 255.0;  return; }
  ...
  // r/255 = v*(1-s)+v*s*(1-h),  g/255 = v*(1-s)+v*s*h,  b/255 = v*(1-s)
  r -= b;  g -= b;  r += g;
  // r/255 = v*s,  g/255 = v*s*h,  b/255 = v*(1-s)
  h = (h + double(g) / double(r)) / 3.0;
  if (h == 1.0) h = 0.0;
  b += r;
  // r/255 = v*s,  g/255 = v*s*h,  b/255 = v
  v = double(b) / 255.0;
  s = double(r) / double(b);
}
...
// Initializes cubic interpolation at location 'i + t1', '0 <= t1 <= 1':
...
double Mesh::CellError(long ind[9]) {
  ...
  double rx,ry,ux,uy,vx,vy,c,s,phi,alpha,beta,ax,ay,bx,by,dx,dy,err;
  int    i;
...
void Mesh::Seed() {
  ...
  for (i=0; i<n; i++) {
    ...
    for (j=0; j<n; j++) {
      ...
      ind[k] = j;  value[k] = v;
      if (m < 9) m++;  // Up to nine nearest points are saved.
    }
...
void Mesh::FindIndices() {
  ...
  long actBeg     = 0,  // Marks the range of extension
       ...
       actSync    = 8, i,j,k,kk,m, dx,dy,rc,r;
  int  changed    = 0;  // To prevent infinite looping.
  // grid indices of seed cell are set to '-1,0,1':
  for (i=0; i<9; i++) {
    ix[i]     = ix_init[i];
    iy[i]     = iy_init[i];
    active[i] = i + 1;  // The outer 8 points are extension points.
  }
  // Hash table init:
      ...
      if (j < 4) {
        ...
        for (j=0; j<4; j++) {
          if ((k = work[j]) >= 0) {
            ...
            if (kk >= 0 && m >= 0) {
              nc++;  xc += x[k] + x[kk] - x[m];  yc += y[k] + y[kk] - y[m];
        } } }
        ...
        if (nc) {
          ...
          if (r <= rc / 10) {
            ...
            Insert();  // Insert point 'n' into the hash table.
        } }
        else redo = 1;  // Signal reinsertion of current point in active list.
    } }
...
void Picture::FilterRedPoints() {
  ...
  double   pHue, h,s,v,m,mz,z;
  int      mode;
  pixel_t *raw;
  long     nRaw;
  if (pRed == pGreen && pGreen == pBlue) {
    mode = 2;
    Print("Hue specification '-pc' ignored as gray value!\n");
  } else mode = 0;
  for (; mode < 4; mode++) {
    switch (mode) {
      case 0:  convertRGBtoHSV(pRed,pGreen,pBlue,pHue,s,v);
               m = 1e-8;
               break;
    ...
    for (nRaw=size,raw=pixel; nRaw; nRaw-=3,raw+=3) {
      convertRGBtoHSV(raw[0],raw[1],raw[2],h,s,v);
      if (mode < 2) {
...
int Picture::SwapRedGreenPoint(long x,long y,int canal) {
  ...
  if ((value = base[canal])) {
    ...
    for (;;) {
      ...
      else border[dir-4] = -1;  // Picture boundary reached.
    }
  ...
  for (ix=-2; ix<=2; ix++) {
    ...
    for (iy=iy0; iy<=iy1; iy++) {
      ...
      if (0 <= xx && xx < width && 0 <= yy && yy < height) {
        ...
        if (white) raw[0] = raw[1] = raw[2] = 255; else {
          if (!raw[0] && !raw[1]) raw[2] = 255;
} } } } }
...
void Picture::CreateMesh(Grid& grid) {
  ...
  mesh.Seed();
  mesh.FindIndices();
  for (i=grid.n; i<grid.nAll; i++) SwapRedGreenPoint(grid.x[i],grid.y[i],1);
  for (i=0; i<mesh.n; i++) {
    if ((k = mesh.Right(i)) >= 0)
...
void Deformation::Flatten(Picture& srcPic, Picture& dstPic) {
  long      srcMX  = srcPic.width  - 1, srcOX = 3,                srcIX,
            srcMY  = srcPic.height - 1, srcOY = srcPic.width * 3, srcIY,
            defMX  = nx            - 1, defOX = 2,                defIX,
            defMY  = ny            - 1, defOY = nx * 2,           defIY,
  ...
  double    dstSX = double(defMX) / double(dstMX),
            ...
            srcSY = double(srcMY) / double(0x10000000), ti,tf;
  pixel_t  *srcRaw,*dstRaw;
  deform_t *mapY,*mapX;
  Cubic2<deform_t> defCub(defMX,defOX,defMY,defOY);
  Cubic2<pixel_t>  srcCub(srcMX,srcOX,srcMY,srcOY);
  dstPic.Reset(dstMX+1,dstMY+1);
  // Empty all pixel values with color white as default value:
  i = dstPic.size;  dstRaw = dstPic.pixel;
  while (i--) *dstRaw++ = 255;
  for (dstY=0,dstRaw=dstPic.pixel; dstY<=dstMY; dstY++) {
    defIY = long(ti = floor(tf = dstSY * dstY));
    defCub.cy(defIY,tf - ti);
    mapY = xyMap + defOY * defIY;
    for (dstX=0; dstX<=dstMX; dstX++,dstRaw+=3) {
      defIX = long(ti = floor(tf = dstSX * dstX));
      defCub.cx(defIX,tf - ti);
      mapX = mapY + 2 * defIX;
      tf = srcSX * defCub(mapX);
      if (tf >= 0.0 && tf <= double(srcMX)) {
        srcIX = long(ti = floor(tf));
        srcCub.cx(srcIX,tf - ti);
        tf = srcSY * defCub(mapX+1);
        if (tf >= 0.0 && tf <= double(srcMY)) {
          srcIY = long(ti = floor(tf));
          srcCub.cy(srcIY,tf - ti);
          srcRaw = srcPic.pixel + (srcOY * srcIY + srcOX * srcIX);
          for (i=0; i<3; i++) {
            tf = round(srcCub(srcRaw+i));
            if      (tf < 0.0)   tf = 0.0;
            else if (tf > 255.0) tf = 255.0;
            dstRaw[i] = pixel_t(tf);
} } } } } }
